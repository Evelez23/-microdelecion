// js/game-complete.js - TODO el código JavaScript corregido con sonidos
...
window.addEventListener('load', function() {
    ...
    const imageUrls = {
      ...
      uni_run: `${baseUrl}Amigos/unirun.png`,        // ¡CORREGIDO!
    ...
    let gameState = {
      ...
      level: 1,
      gameStarted: false,
      soundEnabled: true  // Sonidos activados por defecto
    };
    ...
    function playSound(soundName) {
        if (gameState.soundEnabled && sounds[soundName]) {
            try {
                // Para sonidos cortos (todos excepto background)
                ...
                // Para música de fondo
                    ...
                    sounds.background.play().catch(e => {
                        console.log("Error autoplay música: ", e);
                    });
                }
            } catch (error) {
    ...
    function update() {
        ...
        // ¡CORREGIDO! Ahora puede abrazar incluso con unicornio
        ...
        if (allHugged && friends.length > 0) {
            ...
            // Desmontar al oso del unicornio al cambiar de nivel
            if (oso.hasUnicorn) {
                oso.hasUnicorn = false;
                oso.width = 80;
                oso.height = 80;
                unicornPowerElement.style.display = 'none';
                showHint("¡El unicornio mágico se ha ido! Busca otro en este nivel", 3000);
            }
    ...
    function drawOsoSprite() {
        ...
        if (oso.hasUnicorn) {
            if (oso.hugging) {
                spriteName = "oso_hugging"; // Usar sprite de abrazo aunque tenga unicornio
            } else {
    ...
    // Verificar estado de sonidos después de 5 segundos
    setTimeout(() => {
        console.log("=== ESTADO DE SONIDOS ===");
        for (let key in sounds) {
            if (sounds[key]) {
                console.log(`${key}: ${sounds[key].readyState} (${soundUrls[key]})`);
            } else {
                console.log(`${key}: NO CARGADO`);
            }
        }
    }, 5000);
