async function initStats(){
  ...
  const h = data.filter(r => (r.genero ||'').toUpperCase()==='M' || (r.genero ||'').toUpperCase() === 'MASCULINO').length;
  const m = data.filter(r => (r.genero ||'').toUpperCase()==='F' || (r.genero ||'').toUpperCase() === 'FEMENINO').length;
